                        ...
                        <div class="row g-0">
                            ...
                            <div class="col-6 d-flex align-items-center border-top py-2 mt-3 ">
                                <div class="border border-0 bg-info px-2" style="border-radius: 30px;">
                                    <a href="
                                            @if (session()->get('uname') !== null)
                                            {{ route('posts.upvote', ['user_id' => session()->get('id'), 'post_id' => $post->id]) }}
                                            @else
                                            {{ route('login.index') }}
                                            @endif
                                            ">

                                    <i class="fas fa-arrow-alt-circle-up me-1"></i>{{ count($post->upvotes) }} <i class="text-muted"></a>|
                                        <a href=" @if (session()->get('uname') !== null)
                                            {{ route('posts.downvote', ['user_id' => session()->get('id'), 'post_id' => $post->id]) }}
                                            @else
                                            {{ route('login.index') }}
                                            @endif
                                            "></i>
                                    <i class="fas fa-arrow-alt-circle-down ms-1"></i> {{ count($post->downvotes) }}
                                    </a>
                                </div>
                                <div class="mx-3"><i
